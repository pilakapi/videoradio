...
// PIN de acceso configurable (por defecto 198823)
const ACCESS_PIN = process.env.ACCESS_PIN || '198823';
...
const authenticatePin = (req, res, next) => {
  const authHeader = req.headers['x-pin'];
  
  if (!authHeader) {
    return res.status(401).json({ error: 'PIN requerido' });
  }
  
  if (authHeader !== ACCESS_PIN) {
...
// Verificar PIN
app.post('/api/auth/verify', authenticatePin, (req, res) => {
...
async function initDatabase() {
  try {
    // Tabla de streams (sin usuario, solo un PIN global)
    await pool.query(`
...
// Rutas de streams (protegidas por PIN)
app.get('/api/streams', authenticatePin, async (req, res) => {
  try {
...
app.post('/api/streams', authenticatePin, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'INSERT INTO streams (name, slug, video_url, radio_url) VALUES ($1, $2, $3, $4) RETURNING *',
      [name, slug, video_url, radio_url]
    );
...
app.put('/api/streams/:id', authenticatePin, async (req, res) => {
  try {
    const { id } = req.params;
    const { name, video_url, radio_url } = req.body;
...
app.delete('/api/streams/:id', authenticatePin, async (req, res) => {
  try {
    const { id } = req.params;
...
function clearStreamCache(streamId) {
  ...
  const streamDir = path.join(__dirname, 'public', 'stream', streamId.toString());
...
function generateHLSStream(streamId, videoUrl, radioUrl) {
  return new Promise((resolve, reject) => {
    const outputDir = path.join(__dirname, 'public', 'stream', streamId.toString());
...
// Playlist M3U para el stream (público, sin protección)
app.get('/u/:channel.m3u', async (req, res) => {
  try {
...
app.listen(PORT, async () => {
  ...
  console.log(`🔐 PIN de acceso: ${ACCESS_PIN}`);
  await initDatabase();
});
