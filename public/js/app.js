...
function getToken() {
  return localStorage.getItem('streamMixerToken');
}

function getUser() {
  const user = localStorage.getItem('streamMixerUser');
  return user ? JSON.parse(user) : null;
}

function logout() {
  localStorage.removeItem('streamMixerToken');
  localStorage.removeItem('streamMixerUser');
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  loginForm.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      localStorage.setItem('streamMixerToken', data.token);
      localStorage.setItem('streamMixerUser', JSON.stringify(data.user));
...
function renderStreams() {
  ...
  grid.innerHTML = streams.map(stream => {
    ...
    const m3uUrl = `${baseUrl}/u/${getUser().username}/${stream.slug}.m3u`;
